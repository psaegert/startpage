...
function displayWeather(){
    ...
    fill(colors.white);
    // console.log(weather);
    // text(weather.temp + " C", width/2, height/2);
    // text(weather.descr, width/2, height/2 + 20);
    // weather.icon = "FOG";
    snowing = false;
    switch(weather.icon){
    ...
    for(let i = 0; i < 3; i++){
        ...
        switch(i){
            case 0:
                if(weather.hum > 0){
                    stroke(colors.hum);
                    arc(x, y, 12, 12, - HALF_PI, TWO_PI * weather.hum - HALF_PI);
                }
                break;
            case 1:
                if(weather.clcover > 0){
                    stroke(colors.clcover);
                    arc(x, y, 12, 12, - HALF_PI, TWO_PI * weather.clcover - HALF_PI);
            ...
            case 2:
                if(weather.uv > 0){
                    stroke(colors.uv);
                    arc(x, y, 12, 12, - HALF_PI, TWO_PI * weather.uv - HALF_PI);
